...
final class GameLibrary: ObservableObject {
    ...
    // This must only stop the modal loop; the actual siglus entry is called outside SwiftUI.
    var onLaunchRequest: ((GameEntry) -> Void)? = nil

    private let fm = FileManager.default

    private var libraryURL: URL {
        // ~/Library/Application Support/SiglusLauncher/library.json
    ...
    private func prepareImport(url: URL) {
        let title = siglusGameName(for: url)
        let path = url.path
        let id = stableId(for: path)
        let coverPath = siglusGameCoverPath(for: url)
