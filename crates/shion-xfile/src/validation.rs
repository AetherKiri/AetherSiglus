use crate::semantic::{
    Animation, CompressedAnimationSet, EffectInstance, Frame, Material, Mesh, PMInfo, PatchMesh,
    Scene,
};
...
pub fn validate_scene(scene: &Scene) -> ValidationReport {
    ...
    for (index, patch_mesh) in scene.loose_patch_meshes.iter().enumerate() {
        validate_patch_mesh(
            patch_mesh,
            &format!("Scene/LoosePatchMesh[{index}]"),
            &mut report,
        );
    }
    ...
    for (index, material) in scene.loose_materials.iter().enumerate() {
        validate_material(
            material,
            &format!("Scene/LooseMaterial[{index}]"),
            &mut report,
        );
    }
    ...
    for (index, effect) in scene.loose_effect_instances.iter().enumerate() {
        validate_effect_instance(
            effect,
            &format!("Scene/LooseEffectInstance[{index}]"),
            &mut report,
        );
    }

    for (index, compressed) in scene.compressed_animation_sets.iter().enumerate() {
        validate_compressed_animation_set(
            compressed,
            &format!("Scene/CompressedAnimationSet[{index}]"),
            &mut report,
        );
    }
...
fn report_duplicate_frame_names(
    frame_names: &HashMap<String, usize>,
    ...
    for (name, count) in frame_names {
        if *count > 1 {
            report.warning(
                "Scene".to_string(),
                format!("frame name '{}' appears {} times", name, count),
            );
        }
    }
}
...
fn validate_mesh(
        ...
        for &vertex_index in face {
            if vertex_index as usize >= mesh.vertices.len() {
                report.error(
                    format!("{path}/Face[{face_index}]"),
                    format!(
                        "vertex index {} is out of range for {} vertices",
                        vertex_index,
                        mesh.vertices.len()
                    ),
                );
    ...
    if let Some(adjacency) = &mesh.face_adjacency {
        ...
        if adjacency.indices.len() != expected {
            report.warning(
                path.to_string(),
                format!(
                    "FaceAdjacency count {} does not match expected 3-per-face count {}",
                    adjacency.indices.len(),
                    expected
                ),
            );
        }
        ...
        && face_wraps.wraps.len() != mesh.faces.len()
    {
        report.warning(
            path.to_string(),
            format!(
                "MeshFaceWraps count {} does not match mesh face count {}",
                face_wraps.wraps.len(),
                mesh.faces.len()
            ),
        );
    }
    ...
    if let Some(duplication) = &mesh.vertex_duplication_indices {
        if duplication.indices.len() != mesh.vertices.len() {
            report.warning(
                path.to_string(),
                format!(
                    "VertexDuplicationIndices count {} does not match vertex count {}",
                    duplication.indices.len(),
                    mesh.vertices.len()
                ),
            );
        }
        for (index, &original) in duplication.indices.iter().enumerate() {
            if original >= duplication.original_vertices {
                report.warning(
                    ...
                    format!(
                        "duplicate source index {} is not smaller than nOriginalVertices {}",
                        original, duplication.original_vertices
                    ),
                );
            }
        }
        ...
        && fvf.dwords.is_empty()
    {
        report.warning(
            path.to_string(),
            "FVFData is present but contains zero DWORD payload entries",
        );
    }
    ...
    if let Some(decl) = &mesh.decl_data {
        if decl.elements.is_empty() {
            report.warning(
                path.to_string(),
                "DeclData is present but contains zero VertexElement entries",
            );
        }
        for (index, element) in decl.elements.iter().enumerate() {
            if element.usage_index > 15 {
                report.warning(
                    ...
                    format!(
                        "usage_index {} exceeds the common Direct3D 9 declaration range 0..=15",
                        element.usage_index
                    ),
                );
            }
        ...
        if decl.dwords.is_empty() {
            report.warning(
                path.to_string(),
                "DeclData is present but contains zero DWORD payload entries",
            );
        }
    }
        ...
        && texcoords.len() != mesh.vertices.len()
    {
        report.warning(
            path.to_string(),
            format!(
                "MeshTextureCoords count {} does not match vertex count {}",
                texcoords.len(),
                mesh.vertices.len()
            ),
        );
        ...
        for (color_index, color) in colors.iter().enumerate() {
            if color.index as usize >= mesh.vertices.len() {
                report.error(
                    ...
                    format!(
                        "vertex color index {} is out of range for {} vertices",
                        color.index,
                        mesh.vertices.len()
                    ),
                );
    ...
    if let Some(normals) = &mesh.normals {
        if normals.face_normals.len() != mesh.faces.len() {
            report.warning(
                path.to_string(),
                format!(
                    "MeshNormals face count {} does not match mesh face count {}",
                    normals.face_normals.len(),
                    mesh.faces.len()
                ),
            );
        }
            ...
            for &normal_index in face {
                if normal_index as usize >= normals.normals.len() {
                    report.error(
                        ...
                        format!(
                            "normal index {} is out of range for {} normals",
                            normal_index,
                            normals.normals.len()
                        ),
                    );
    ...
    if let Some(material_list) = &mesh.material_list {
        if material_list.face_indexes.len() != mesh.faces.len() {
            report.warning(
                path.to_string(),
                format!(
                    "MeshMaterialList face-material count {} does not match mesh face count {}",
                    material_list.face_indexes.len(),
                    mesh.faces.len()
                ),
            );
        }
        ...
        let material_slots =
            material_list.materials.len() + material_list.material_references.len();
        if material_slots > material_list.material_count as usize {
            report.warning(
                path.to_string(),
                format!(
                    "material payload count {} exceeds declared material_count {}",
                    material_slots, material_list.material_count
                ),
            );
        }
        ...
        for (face_index, &material_index) in material_list.face_indexes.iter().enumerate() {
            if material_index >= material_list.material_count {
                report.error(
                    ...
                    format!(
                        "material index {} is out of range for declared material_count {}",
                        material_index, material_list.material_count
                    ),
                );
            }
        }
        ...
        for (index, material) in material_list.materials.iter().enumerate() {
            validate_material(
                material,
                &format!("{path}/MeshMaterialList/Material[{index}]"),
                report,
            );
        }
    }
    ...
    if let Some(header) = &mesh.skin_mesh_header
        && header.bone_count as usize != mesh.skin_weights.len()
    {
        report.warning(
            path.to_string(),
            format!(
                "XSkinMeshHeader bone_count {} does not match SkinWeights object count {}",
                header.bone_count,
    ...
    for (skin_index, skin) in mesh.skin_weights.iter().enumerate() {
        if skin.vertex_indices.len() != skin.weights.len() {
            report.error(
                ...
                format!(
                    "vertex index count {} does not match weight count {}",
                    skin.vertex_indices.len(),
                    skin.weights.len()
                ),
            );
        }
        ...
        for &vertex_index in &skin.vertex_indices {
            if vertex_index as usize >= mesh.vertices.len() {
                report.error(
                    format!("{path}/SkinWeights[{skin_index}]"),
                    format!(
                        "skin vertex index {} is out of range for {} vertices",
                        vertex_index,
                        mesh.vertices.len()
                    ),
                );
        ...
        if !skin.transform_node_name.is_empty()
            && !frame_name_set.contains(skin.transform_node_name.as_str())
        {
            report.warning(
                format!("{path}/SkinWeights[{skin_index}]"),
                format!(
                    "SkinWeights transform node '{}' does not resolve to any frame name",
...
fn validate_effect_instance(effect: &EffectInstance, path: &str, report: &mut ValidationReport) {
    if effect.effect_filename.is_empty() {
        report.warning(
            path.to_string(),
            "EffectInstance has an empty effect filename",
        );
    }
    ...
    for (index, entry) in effect.strings.iter().enumerate() {
        if entry.param_name.is_empty() {
            report.warning(
                format!("{path}/EffectParamString[{index}]"),
                "effect string parameter name is empty",
            );
        }
    }
    ...
    for (index, entry) in effect.dwords.iter().enumerate() {
        if entry.param_name.is_empty() {
            report.warning(
                format!("{path}/EffectParamDWord[{index}]"),
                "effect DWORD parameter name is empty",
            );
        }
    }
    ...
    for (index, entry) in effect.floats.iter().enumerate() {
        if entry.param_name.is_empty() {
            report.warning(
                format!("{path}/EffectParamFloats[{index}]"),
                "effect float parameter name is empty",
            );
        }
        if entry.values.is_empty() {
            report.warning(
                format!("{path}/EffectParamFloats[{index}]"),
                "effect float parameter contains zero values",
            );
        }
    }
}
    ...
    for (patch_index, patch) in patch_mesh.patches.iter().enumerate() {
        if patch.control_indices.is_empty() {
            report.warning(
                ...
                "patch has zero control indices",
            );
        }
        for &control_index in &patch.control_indices {
            if control_index as usize >= patch_mesh.vertices.len() {
                report.error(
                    format!("{path}/Patch[{patch_index}]"),
                    format!(
                        "patch control index {} is out of range for {} vertices",
                        control_index,
                        patch_mesh.vertices.len()
                    ),
                );
            }
        }
    ...
    for (index, range) in pm_info.attribute_ranges.iter().enumerate() {
        if range.faces_min > range.faces_max {
            report.warning(
                ...
                format!(
                    "PMAttributeRange faces_min {} exceeds faces_max {}",
                    range.faces_min, range.faces_max
                ),
            );
        }
...
fn validate_compressed_animation_set(
    ...
    let expected_words = compressed.compressed_block_size.div_ceil(4);
...
fn validate_animation(
    ...
    if let Some(target_name) = &animation.target_name
        && !frame_name_set.contains(target_name.as_str())
    {
        report.warning(
            path.to_string(),
            format!(
                "animation target '{}' does not resolve to any frame name",
                target_name
            ),
        );
    }
    ...
    if animation.keys.is_empty() {
        report.warning(
            path.to_string(),
            "animation contains no AnimationKey blocks",
        );
    }

    for (block_index, block) in animation.keys.iter().enumerate() {
        if !matches!(block.key_type, 0..=4) {
            report.warning(
                format!("{path}/AnimationKey[{block_index}]"),
                format!("unknown animation key type {}", block.key_type),
            );
        }
        ...
        if block.keys.is_empty() {
            report.warning(
                format!("{path}/AnimationKey[{block_index}]"),
                "animation key block contains no keyed samples",
            );
        }
        ...
        for (sample_index, key) in block.keys.iter().enumerate() {
                ...
                && key.time < prev
            {
                report.warning(
                    format!("{path}/AnimationKey[{block_index}]/Key[{sample_index}]"),
                    format!(
                        "animation key time {} is earlier than previous sample {}",
                        key.time, prev
                    ),
                );
            }
