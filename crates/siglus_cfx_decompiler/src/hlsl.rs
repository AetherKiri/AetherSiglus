    ...
    for reg in &ctx.used_outputs {
        for component in 0..vector_len_for_type(output_field_type(*reg)) {
            state.values.insert(
                ...
                scalar_zero(output_field_type(*reg)).to_string(),
            );
        }
    }
}
...
fn fold_instruction(ctx: &Context<'_>, state: &mut FoldedState, inst: &Instruction) {
    match inst.opcode {
        ...
        _ if inst.opcode.has_destination() => {
            let Some(dst_reg) = inst.dest_register() else {
                return;
            };
            ...
            for (write_index, dst_component) in dst_components.iter().copied().enumerate() {
                if let Some(mut expr) =
                    folded_instruction_component(ctx, &old, inst, write_index, dst_component)
                {
    ...
    match inst.opcode {
        Opcode::Def => {
            for component in 0..4 {
                state.values.insert(
                    ComponentKey { reg, component },
                    fmt_f32(inst.get_float_param(component + 1)),
                );
            }
        }
        Opcode::DefI => {
            for component in 0..4 {
                state.values.insert(
                    ComponentKey { reg, component },
                    inst.get_int_param(component + 1).to_string(),
                );
            }
        }
        Opcode::DefB => {
            state.values.insert(
                ComponentKey { reg, component: 0 },
                if inst.get_int_param(1) != 0 {
                    "true".to_string()
                } else {
                    "false".to_string()
                },
...
fn folded_instruction_component(
    ...
    let c = dst_component;
    let s =
        |param: usize, component: usize| source_component_expr(ctx, old, inst, param, component);
    Some(match inst.opcode {
        ...
        Opcode::Dp3 => {
            dot_expr_from_components(|component| s(1, component), |component| s(2, component), 3)
        }
        Opcode::Dp4 => {
            dot_expr_from_components(|component| s(1, component), |component| s(2, component), 4)
        }
        ...
        Opcode::Crs => {
            cross_component_expr(|component| s(1, component), |component| s(2, component), c)
        }
...
fn component_value_from(
    ...
    match reg.ty {
        RegisterType::Input | RegisterType::Texture | RegisterType::MiscType => format!(
            "input.{}{}",
            input_field_name(reg),
            component_suffix(component)
        ),
        RegisterType::Const => const_component_expr(ctx, reg.number, component),
        RegisterType::ConstInt => format!(
            "{}{}",
            int_const_expr(ctx, reg.number),
            component_suffix(component)
        ),
...
fn texture_component_expr(
    ...
    let sample = match inst.opcode {
        Opcode::TexLdd => {
            let ddx = (0..dim)
                .map(|i| source_component_expr(ctx, old, inst, 3, i))
                .collect::<Vec<_>>();
            let ddy = (0..dim)
                .map(|i| source_component_expr(ctx, old, inst, 4, i))
                .collect::<Vec<_>>();
            format!(
                "{}({}, {}, {}, {})",
                base,
                sampler_name,
                coord_expr,
                vector_constructor(&ddx),
                vector_constructor(&ddy)
            )
        }
    ...
    for inst in &shader.instructions {
        ...
        for pi in first_src..inst.params.len() {
            if inst.opcode == Opcode::Dcl
                || inst.opcode == Opcode::Def
                || inst.opcode == Opcode::DefI
                || inst.opcode == Opcode::DefB
            {
                continue;
            }
...
fn emit_main(out: &mut String, ctx: &Context<'_>) {
    ...
    for reg in &ctx.used_outputs {
        out.push_str(&format!(
            "    output.{} = {};\n",
            output_field_name(*reg),
            zero_value(output_field_type(*reg))
        ));
    }
    ...
    match inst.opcode {
        ...
        Opcode::TexKill => {
            if let Some(reg) = inst.dest_register() {
                let expr = format!(
                    ...
                    hlsl_mask_suffix(inst.dest_write_mask())
                );
        ...
        Opcode::IfC => {
            ...
            line(
                out,
                *indent,
                &format!("if (all({} {} {})) {{", a, cmp_op(inst.comparison()), b),
            );
            *indent += 1;
        }
        Opcode::Else => {
        ...
        Opcode::BreakC => {
            ...
            line(
                out,
                *indent,
                &format!("if (all({} {} {})) break;", a, cmp_op(inst.comparison()), b),
            );
        }
        Opcode::Rep => {
            ...
            line(
                ...
                *indent,
                &format!(
                    "for (int _rep{} = 0; _rep{} < (int)({}); ++_rep{}) {{",
                    inst.offset, inst.offset, n, inst.offset
                ),
            );
            *indent += 1;
        }
...
fn texture_expr(ctx: &Context<'_>, inst: &Instruction) -> String {
    ...
    let sampler_ty = sampler_texture_type(ctx, sampler);
    let dim = sampler_ty.hlsl_dim();
    let coord = source_expr(
        ctx,
        inst,
        1,
        if inst.opcode == Opcode::Tex { dim } else { 4 },
    );
    let base = texture_intrinsic(inst, sampler_ty);
    match inst.opcode {
        Opcode::TexLdd => {
...
fn int_const_expr(ctx: &Context<'_>, index: u16) -> String {
    if let Some(ctab) = ctx.ctab
        && let Some(c) = ctab
            .constants
            .iter()
            .find(|c| c.register_set == RegisterSet::Int4 && c.register_index == index)
    {
...
fn bool_const_expr(ctx: &Context<'_>, index: u16) -> String {
    if let Some(ctab) = ctx.ctab
        && let Some(c) = ctab
            .constants
            .iter()
            .find(|c| c.register_set == RegisterSet::Bool && c.register_index == index)
    {
        return sanitize_ident(&c.name);
    }
    format!("b{}", index)
