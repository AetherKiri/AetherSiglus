...
impl<'a> Reader<'a> {
    ...
    pub fn read_u8(&mut self) -> Result<u8> {
        if self.pos >= self.data.len() {
            return Err(Error::with_offset(
                "unexpected end while reading u8",
                self.pos,
            ));
        }
    ...
    pub fn read_u32(&mut self) -> Result<u32> {
        ...
        let b = self
            .read_bytes(4)
            .map_err(|_| Error::with_offset("unexpected end while reading u32", off))?;
    ...
    pub fn read_u16(&mut self) -> Result<u16> {
        ...
        let b = self
            .read_bytes(2)
            .map_err(|_| Error::with_offset("unexpected end while reading u16", off))?;
    ...
    pub fn read_bytes(&mut self, n: usize) -> Result<&'a [u8]> {
        let end = self
            .pos
            .checked_add(n)
            .ok_or_else(|| Error::with_offset("byte range overflow", self.pos))?;
        if end > self.data.len() {
            return Err(Error::with_offset(
                "unexpected end while reading bytes",
                self.pos,
            ));
        }
...
pub fn checked_range(
    ...
    let end = start
        .checked_add(len)
        .ok_or_else(|| Error::new(format!("{what} range overflow")))?;
    if end > data_len {
        return Err(Error::new(format!(
            "{what} range 0x{start:X}..0x{end:X} is outside input size 0x{data_len:X}"
        )));
    }
    Ok(start..end)
}
