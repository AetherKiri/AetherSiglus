...
mod create_pct_file_name_tests {
    use super::{
        ObjectState, set_object_button_param_like_cpp, split_create_pct_file_name_like_cpp,
    };
...
fn dispatch_embedded_object_child_item_op(
    ...
    if sg_mwnd_object_trace_enabled() {
        sg_mwnd_object_trace(format!(
            ...
            parent_runtime_slot, child_u, child_runtime_slot, child_op, child_tail
        ))
    };
...
fn load_siglus_emote_runtime(
    ...
    for name in file_name.split('|') {
        anyhow::ensure!(
            !name.is_empty() && sources.len() < 64,
            "invalid E-mote source list"
        );
    ...
    for (glyph_index, glyph) in positioned.into_iter().enumerate() {
        ...
        let local = |render: Option<crate::text_render::PositionedTextRender>| {
            (
                glyph
                    .x
                    .saturating_add(render.map(|r| r.offset_x).unwrap_or(0)),
                glyph
                    .y
                    .saturating_add(render.map(|r| r.offset_y).unwrap_or(0)),
            )
        };
...
fn restore_object_backend_after_load(
    ...
    if let Err(error) = obj
        .gan
        .restore_resource(&ctx.project_dir, &ctx.globals.append_dir)
    {
...
fn name_debug(script_args: &[Value]) -> Option<String> {
    script_args.first().and_then(|v| match v {
...
fn try_object_fast_lane(
    ...
    if handled { Some(true) } else { None }
}
    ...
    if op == 93 {
            ...
            && (tail[0] == -1 || tail[0] == ctx.ids.elm_array || tail[0] == super::codes::ELM_ARRAY)
        {
            ...
            let leaf = if tail.len() == 3 {
                ...
                && (tail[3] == -1
                    || tail[3] == ctx.ids.elm_array
                    || tail[3] == super::codes::ELM_ARRAY)
            {
...
mod message_block_savepoint_tests {
    use super::{MwndState, start_mwnd_msg_block_if_needed};
...
pub fn dispatch(ctx: &mut CommandContext, args: &[Value]) -> Result<bool> {
    ...
    {
        if let Some(true) =
            try_object_fast_lane(ctx, args, chain.to_vec(), chain_pos, al_id, ret_form)
        {
            return Ok(true);
        }
    }
