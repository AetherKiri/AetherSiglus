...
use crate::text_render::{FontCache, TextStyle, embedded_default_font_names};
...
fn accumulate_integer_millis(elapsed: std::time::Duration, sub_ms_ns: &mut u32) -> i32 {
    const NS_PER_MS: u128 = 1_000_000;
    let total_ns = elapsed.as_nanos().saturating_add(u128::from(*sub_ms_ns));
...
impl CommandContext {
    ...
    pub fn needs_continuous_frame(&self) -> bool {
        ...
        if !self.globals.pending_frame_action_finishes.is_empty()
            || !self.globals.pending_button_actions.is_empty()
        {
            return true;
        }
        if self
    ...
    fn runtime_is_skip_unread_msg(&self) -> bool {
        let configured = self
            ...
            .copied()
            .unwrap_or({
                if self.globals.syscom.original_config.skip_unread_message_flag {
                    1
                } else {
                    0
                }
    ...
    fn runtime_read_skip_is_enable(&self) -> bool {
        ...
        f.enable && f.exist && (self.runtime_is_skip_unread_msg() || self.read_skip_enable_flag)
    }
    ...
    fn process_script_skip_trigger_decide(&mut self, vk: u8) -> bool {
        ...
        if self.wait.message_reveal_waiting() && !self.ui.message_wait_text_fully_revealed() {
            if self.globals.script.not_skip_msg_by_click || !self.input.use_vk_down_up_stock(vk) {
                return false;
            }
    ...
    pub(crate) fn koe_jitan_rate(&self, explicit: Option<bool>, replay: bool) -> u16 {
        ...
        cfg(GET_JITAN_SPEED, original.jitan_speed).clamp(100, 400) as u16
    }
    ...
    fn initialize_flag_lists(&mut self) {
        ...
        for form in [
            ...
            let list = self.globals.int_lists.entry(form as u32).or_default();
        ...
        for form in [
            ...
            let list = self.globals.str_lists.entry(form as u32).or_default();
    ...
    fn gameexe_color(&self, color_no: i64) -> (u8, u8, u8) {
        if color_no >= 0
            && let Some(&c) = self.tables.color_table.get(color_no as usize)
        {
            return c;
        }
        (255, 255, 255)
    ...
    fn load_mouse_cursor_runtime(&mut self, cursor_no: i64) -> Option<&MouseCursorRuntime> {
        ...
        if !self.mouse_cursor_cache.contains_key(&key)
            && let Some(loaded) = self.load_mouse_cursor_runtime_uncached(cursor_no)
        {
    ...
    fn append_mouse_cursor_sprite(&mut self, list: &mut Vec<RenderSprite>) {
        ...
        let mut sprite = Sprite {
            ...
            x: self.input.mouse_x.saturating_sub(frame.hot_x),
            y: self.input.mouse_y.saturating_sub(frame.hot_y),
            alpha: 255,
            tr: 255,
            alpha_blend: true,
            alpha_test: false,
    ...
    #[doc(hidden)]
    pub fn install_scene_metadata(&self, append_dir: &str, pck: &ScenePck) -> Result<()> {
    ...
    pub(crate) fn scene_metadata(&self) -> Result<Arc<SceneMetadata>> {
        ...
        if let Some((cached_append, metadata)) = self.scene_metadata.borrow().as_ref()
            && cached_append.eq_ignore_ascii_case(&active_append)
        {
    ...
    pub fn reset_for_scene_restart(&mut self) {
        ...
        self.globals
            .str_lists
            .insert(codes::ELM_GLOBAL_S as u32, vec![String::new(); local_count]);
    ...
    fn update_object_button_hover(&mut self) {
...

        {
            ...
            for stage_idx in &stage_ids {
                ...
                for (obj_idx, obj) in objs.iter_mut().enumerate() {
                    if embedded_by_stage
                        .get(stage_idx)
                        .is_some_and(|slots| slots.contains(&obj_idx))
                    {
                        continue;
                    }
            ...
            for stage_idx in group_stage_ids {
                ...
                for (group_idx, g) in groups.iter_mut().enumerate() {
                    ...
                    for (obj_idx, obj) in objs.iter_mut().enumerate() {
                        ...
                        if let Some((button_no, slot)) =
                            pushed_group_button_recursive(obj_idx, obj, group_idx)
                        {
            ...
            if !standalone_tied && let Some(hit) = standalone_best {
                if hit.event_enabled && play_hover_sound && !hit.was_hit {
                    hit_sounds.push(hit.se_no);
                }
                for stage_idx in &stage_ids {
            ...
            match b {
                    ...
                    for stage_idx in group_stage_ids {
                        ...
                        for (group_idx, g) in groups.iter_mut().enumerate() {
                            ...
                            if g.pushed_runtime_slot != Some(hit_slot)
                                && let Some(objs) = object_lists.get(&stage_idx)
                                && let Some(se_no) =
                ...
                input::VmMouseButton::Right => {
                    ...
                    if let Some((_priority, group_idx, stage_idx)) = candidates.first().copied()
                        && let Some(groups) = group_lists.get_mut(&stage_idx)
                        && let Some(g) = groups.get_mut(group_idx)
                    {
    ...
    pub fn on_key_down(&mut self, k: input::VmKey) {
        ...
        if !handled_mwnd_selection
            && let Some((form_id, stage_idx, group_idx)) = self.globals.focused_stage_group
            && let Some(st) = self.globals.stage_forms.get_mut(&form_id)
            && let Some(list) = st.group_lists.get_mut(&stage_idx)
            && let Some(g) = list.get_mut(group_idx)
        {
            match k {
                input::VmKey::Enter => {
    ...
    pub fn on_key_up(&mut self, k: input::VmKey) {
        ...
        if let Some(vk) = input::vmkey_to_vk_code(k) {
            match k {
                ...
                input::VmKey::Escape if self.input.vk_down_up_stock(vk) => {
    ...
    pub fn on_mouse_up(&mut self, b: input::VmMouseButton) {
        ...
        if matches!(b, input::VmMouseButton::Left) && self.process_script_skip_trigger_decide(0x01)
        {
            return;
        }
        if left_down_up && !self.advance_message_wait(true) {
            self.notify_wait_key();
        }
    }
    ...
    fn finish_skipped_movie_waits(&mut self) {
        while let Some(info) = self.wait.take_movie_skip() {
            ...
            if let globals::ObjectBackend::Movie {
                ...
                && let Some(layer) = self.layers.layer_mut(layer_id)
    ...
    fn update_skip_frame_rate_sample(&mut self, real_delta_ms: i32) {
        ...
        if let Some(sample) = (self.frame_rate_bucket_frame_cnt as u64).checked_div(periods) {
            let sample = sample.min(i32::MAX as u64) as i32;
    ...
    fn resolve_mask_image(&mut self, name: &str) -> Option<ImageHandle> {
        ...
        if let Some(path) = resolve_mask_path(&self.project_dir, name)
            && let Ok(id) = self.images.load_file(&path, 0)
        {
            return Some(id);
        }
                    ...
                    globals::ObjectBackend::Gfx => {
                            ...
                            self.gfx.object_sprite_binding(*stage_idx, obj_idx as i64)
                        {
                            ...
                            if let Some(layer) = self.layers.layer_mut(lid)
                                && let Some(spr) = layer.sprite_mut(sid)
                            {
                                spr.visible = false;
                            }
                        }
            ...
            .map(|st| {
                st.ordered_history_indices()
                    ...
                    .filter(|&i| {
                        st.history
                            .get(i)
                            .is_some_and(Self::msg_back_entry_has_content)
                    })
                    .collect()
            })
            .unwrap_or_default()
    ...
    fn msg_back_measure_entry_text(
        ...
        for ch in text.chars() {
            ...
            if x.saturating_add(this_check_size) > msg_w.saturating_add(moji_size)
                || (x.saturating_add(this_check_size) > msg_w
    ...
    fn replay_msg_back_koe(&mut self, history_index: usize) {
        ...
        let index = usize::try_from(entry.koe_play_no)
            .ok()
            .filter(|index| *index < entry.koe_no_list.len())
            .unwrap_or(0);
    ...
    fn sync_selbtn_item_selection(&mut self) {
        if let Some(st) = self
            ...
            .stage_forms
            .get_mut(&self.ids.form_global_stage)
            && let Some(items) = st.btnselitem_lists.get_mut(&TNM_STAGE_FRONT_I64)
        {
    ...
    fn sync_global_movie(&mut self) {
        ...
        let small_rewind = !polled.frame_idx_is_decode_order
            && last_frame_idx
                .is_some_and(|last| frame_idx < last && frame_idx.saturating_add(1) >= last);
        let show_new = frame_idx_changed && !small_rewind;
        let img_id = if let Some(id) = image_id {
            if show_new {
    ...
    fn build_render_list_pre_wipe(
        ...
        let base = self.layers.render_list_excluding(|layer_id, sprite_id| {
            object_keys.contains(&(layer_id, sprite_id))
        });
        let (mut list, debug_lines) =
            build_siglus_object_render_list(self, &base, TNM_STAGE_FRONT_I64, &mut object_keys);
    ...
    fn render_frame_with_effects_inner(&mut self, include_mouse_cursor: bool) -> RenderFrame {
        ...
        let frame = if let Some(wipe_state) = self.globals.wipe.as_ref().cloned() {
            ...
            let (mut next_list, next_debug_lines) =
                build_siglus_object_render_list(self, &base, TNM_STAGE_NEXT_I64, &mut object_keys);
    ...
    // ImageHandle hashes its immutable key and album identity, never the mutable pixels.
...
fn button_real_state_for_visual(
    ...
    if let Some(reason) =
        button_disabled_reason(syscom, obj, mwnd_button_idx, read_skip_runtime_enabled)
    {
        if sg_debug_enabled() {
            eprintln!(
...
fn find_button_se_no_by_runtime_slot_recursive(
    ...
    if object_runtime_slot(obj_idx, obj) == runtime_slot {
        return (obj.button.enabled && obj.button.action_no >= 0).then_some(obj.button.se_no);
    }
    for (child_idx, child) in obj.runtime.child_objects.iter().enumerate() {
        if let Some(se_no) =
...
fn pushed_group_button_recursive(
    ...
    if obj.button.enabled && obj.button.pushed && obj.button.group_idx() == Some(group_idx) {
    ...
    obj.runtime
        ...
        .iter()
        .enumerate()
        .find_map(|(idx, child)| pushed_group_button_recursive(idx, child, group_idx))
}
...
fn hit_test_render_sprite(
    ...
    if let Some(clip) = sprite.dst_clip
        && (mx < clip.left || my < clip.top || mx >= clip.right || my >= clip.bottom)
    {
        return false;
    }
    let image = sprite
...
fn hit_test_standalone_action_button_recursive(
    ...
    fn recurse(
        ...
        if let Some(owner) = effective_owner
            ...
            && let Some(sort_key) = object_button_hit_sort_key_from_render(
                images,
                layers,
                gfx,
                ids,
...
fn sync_emote_object_recursive(
    ...
    if obj.object_type == 12 {
        ...
        if let Some(runtime) = obj.emote.runtime.as_mut()
            && let Err(err) = runtime.set_face_talk(mouth)
        {
...
fn sync_movie_object_recursive(
        ...
        && let Some(file_name) = obj.file_name.clone()
    {
        if trace {
            eprintln!(
                "[SG_MOVIE_TRACE] enter stage={} obj={} file={} playing={} pause={} backend={:?} children={}",
                stage_idx,
                obj_idx,
                file_name,
                obj.movie.playing,
        ...
        if obj.movie.just_finished {
            ...
            if obj.movie.auto_free_flag {
                ...
                if let globals::ObjectBackend::Movie {
                    ...
                    && let Some(layer) = layers.layer_mut(layer_id)
                    && let Some(sprite) = layer.sprite_mut(sprite_id)
                {
                    sprite.visible = false;
                    sprite.image_id = None;
            ...
            && let Some(id) = obj.movie.audio_id.take()
        {
            movie_mgr.stop_audio(id);
        }
        ...
        if obj.object_type == 9 {
                            ...
                            *height = frame.height;
                            ...
                            if trace {
                                eprintln!(
                                    ...
                                    stage_idx, obj_idx, file, frame.width, frame.height,
                                );
                            }
                        }
                        Err(err) => {
            ...
            if obj.movie.playing && obj.movie.audio_id.is_none() {
                if let Some(track) = polled.audio.as_ref() {
                    if let Ok(id) =
                        movie_mgr.start_audio(audio, track, obj.movie.timer_ms, obj.movie.loop_flag)
                    {
...
fn apply_object_tonecurves_recursive(
    ...
    if tonecurve_no >= 0
        && let Some((tonecurve_image_id, tonecurve_row, tonecurve_sat)) =
...
fn apply_gan_effects_recursive(
    ...
    if let Some(pat) = obj.gan.current_pat()
        && !(pat.pat_no == 0 && pat.x == 0 && pat.y == 0 && pat.tr == 255)
    {
...
fn effective_object_info(
    ...
    let dst_clip = if base.clip_use != 0 {
        Some(ClipRect {
            ...
            bottom: event_or_base!(ids.obj_clip_bottom, ids.obj_clip_bottom_eve, clip_bottom)
                as i32,
        })
    } else {
        None
...
fn append_object_tree_nodes(
    ...
    let info =
        precomputed_info.unwrap_or_else(|| effective_object_info(ctx, stage_idx, obj_idx, obj));
    ...
    if visible {
        if obj.object_type == 4 {
            ...
            for mut rs in bound.drain(..) {
                ...
                let (local_x, local_y) = object_backend_sprite_local_offset(obj, rs.sprite_id);
                if local_x != 0 || local_y != 0 {
    ...
    let tree_sort_type = if matches!(obj.object_type, 3..=5) {
...
fn collect_button_visuals_recursive(
    ...
    for (child_idx, child) in obj.runtime.child_objects.iter().enumerate() {
        collect_button_visuals_recursive(ctx, st, stage_idx, child_idx, child, map, None);
    }
}
...
mod button_visual_ownership_tests {
    ...
    fn picture(sprite_id: SpriteId, file: &str) -> globals::ObjectState {
        globals::ObjectState {
            ...
            backend: globals::ObjectBackend::Rect {
                layer_id: 0,
                sprite_id,
                width: 360,
                height: 48,
    ...
    fn save_slot_button_does_not_replace_child_text_digits_or_icons() {
        ...
        let mut text = globals::ObjectState {
            used: true,
            backend: globals::ObjectBackend::String {
                layer_id: 0,
                shadow_sprite_id: 1,
                fuchi_sprite_id: 2,
                sprite_id: 3,
        ...
        let mut number = globals::ObjectState {
            used: true,
            backend: globals::ObjectBackend::Number {
                layer_id: 0,
                sprite_ids: vec![4, 5],
            },
            ..Default::default()
...
fn button_action_pattern(
    ...
    if action_no >= 0
        && let Some(tpl) = tables.button_action_templates.get(action_no as usize)
    {
...
mod skip_state_parity_tests {
    ...
    fn read_skip_button_uses_full_runtime_eligibility() {
        ...
        let mut button = globals::ObjectButtonState {
            sys_type: TNM_SYSCOM_TYPE_READ_SKIP,
            ..Default::default()
        };
...
mod render_tree_fidelity_tests {
    use super::{
        EffectParam, SiglusRenderNode, WipePartition, apply_effects_to_owner,
        classify_wipe_partition, compose_clip_rect, object_alpha_blend_for_render,
        sprite_has_render_payload,
    };
    ...
    fn mesh_payload_does_not_require_a_pct_image() {
        let mut mesh = Sprite {
            mesh_kind: 1,
            mesh_file_name: Some("room.x".to_owned()),
            ..Default::default()
        };
    ...
    fn screen_icon_rotation_keeps_screen_coordinates() {
        ...
        let quad = crate::render_math::sprite_quad_points(
            &sprite, 950.0, 800.0, 132.0, 132.0, 1920.0, 1080.0,
        )
...
mod scene_metadata_cache_tests {
    ...
    fn active_append_change_invalidates_resident_scene_metadata() {
        let mut ctx =
            CommandContext::new(std::env::temp_dir().join("siglus-scene-metadata-cache-test"));
...
mod movie_menu_wait_tests {
    ...
    fn menu_group_ignores_background_clicks_and_skip_until_a_decision() {
        ...
        assert!(
            ctx.wait_poll(),
            "a background click must not choose New Game"
        );
        ...
        assert!(
            !ctx.wait_poll(),
            "a real decision must release the selection wait"
        );
    ...
    fn movie_click_skip_obeys_script_flag_and_requires_down_up() {
        for key_skip in [false, true] {
            ...
            assert_eq!(
                ctx.stack.pop().and_then(|v| v.as_i64()),
                key_skip.then_some(1)
            );
        }
    }
...
mod msg_back_voice_tests {
    ...
    fn backlog_with_button(project: PathBuf, voices: &[(i64, i64)]) -> CommandContext {
        ...
        ctx.globals
            .msgbk_forms
            .insert(ctx.ids.form_global_msgbk, history);
    ...
    fn backlog_voice_click_cycles_clips_and_preserves_message_voice() {
        let mut ctx = backlog_with_button(
            std::env::temp_dir().join("siglus-backlog-click-test"),
            &[(10, 2), (11, 3)],
        );
        ...
        assert_eq!(
            ctx.ui.msg_back_hit_action(128, 55),
            None,
            "clipped part of icon"
        );
        assert_eq!(
            ctx.ui.msg_back_hit_action(128, 62),
            Some(ui::MsgBackHitAction::ReplayKoe(0))
        );
        for (voice, character, next) in [(10, 2, 1), (11, 3, 2), (10, 2, 1)] {
            ...
            ctx.on_mouse_up(input::VmMouseButton::Left);
            assert!(
                ctx.koe.is_playing_any(),
                "click must request voice decoding"
            );
            assert_eq!(ctx.globals.sound_routing.koe_chara_no, character);
            assert!(ctx.globals.sound_routing.koe_ex_flag);
            assert_eq!(
                ctx.globals.int_props[&(constants::fm::GLOBAL as u32)]
    ...
    fn backlog_voice_click_reaches_audio_output() {
        ...
        let voice: i64 = std::env::var("SIGLUS_BACKLOG_TEST_KOE")
            .unwrap()
            .parse()
            .unwrap();
        ...
        assert_eq!(
            ctx.koe.current_koe_no(),
            voice,
            "voice must decode and start"
        );
        assert!(
            ctx.audio.is_enabled(),
            "requires a working audio output device"
        );
        assert!(ctx.koe.is_playing_any());
        std::thread::sleep(std::time::Duration::from_millis(150));
        assert!(
            ctx.koe.current_play_pos_ms() > 0,
            "audio playback must advance"
        );
    }
}
