...
#[cfg(not(all(target_arch = "wasm32", target_os = "unknown")))]
/// Name of the marker that records "this key started the game successfully
/// against these exact resource files".
const KEY_TRUST_FILE: &str = "keytrust.toml";

/// Where the trust marker may live. The game directory first (next to
/// key.toml), then `savedata/`, which is written whenever the player saves and
/// is therefore known to be writable even when the root of the game directory
/// is not.
fn key_trust_candidates(project_dir: &Path) -> [PathBuf; 2] {
    [
        project_dir.join(KEY_TRUST_FILE),
        project_dir.join("savedata").join(KEY_TRUST_FILE),
    ]
}

/// Size + mtime of the resource pair the key has to decrypt. Any change to
/// either file invalidates the trust entry so the key is validated again.
fn key_trust_fingerprint(project_dir: &Path) -> Option<String> {
    let mut parts = Vec::new();
    for path in [
        find_initial_gameexe_path(project_dir).ok(),
        find_scene_pck_path(project_dir).ok(),
    ]
    .into_iter()
    .flatten()
    {
...
fn key_trust_matches(project_dir: &Path, key: &[u8; 16]) -> bool {
    let Some(fingerprint) = key_trust_fingerprint(project_dir) else {
        return false;
    };
    let want: String = key.iter().map(|b| format!("{b:02X}")).collect();
    for path in key_trust_candidates(project_dir) {
        let Ok(text) = std::fs::read_to_string(&path) else {
            continue;
        };
        let mut stored_key = None;
        let mut stored_resources = None;
        for line in text.lines() {
            ...
            match name.trim() {
                "key" => stored_key = Some(value.to_ascii_uppercase()),
                "resources" => stored_resources = Some(value.to_string()),
                _ => {}
            }
        }
        if stored_key.as_deref() == Some(want.as_str())
            && stored_resources.as_deref() == Some(fingerprint.as_str())
        {
            return true;
        }
    }
...
/// Remember that the key currently in use **did start the game**. The host calls
/// this after the engine has been running with a loaded scene, so an entry can
/// never be written for a key that merely passed the structural validator: that
/// validator depends on the LZSS header invariants of a pristine retail
/// `Scene.pck` and false-negatives on repacked resources, which is what made
/// every launch pay for a full, non-converging key recovery.
pub fn record_key_trust(project_dir: &Path) {
    let Some(key) = resolve_project_exe_key(project_dir) else {
        return;
    };
    let Some(fingerprint) = key_trust_fingerprint(project_dir) else {
        return;
    };
    if key_trust_matches(project_dir, &key) {
        return;
    }
    let hex: String = key.iter().map(|b| format!("{b:02X}")).collect();
    let text = format!(
        "# written by siglus_rs after a successful start; delete to force key re-validation\nkey = \"{hex}\"\nresources = \"{fingerprint}\"\n"
    );
    ...
    for path in key_trust_candidates(project_dir) {
        match std::fs::write(&path, &text) {
            Ok(()) => {
                log::warn!(
                    "[SG_KEYTRUST] recorded working key for {} at {}",
                    project_dir.display(),
                    path.display()
                );
                return;
    ...
    if let Some((path, err)) = last_error {
        log::warn!(
            "[SG_KEYTRUST] could not record the working key ({}): {err}",
            path.display()
        );
    }
}
...
fn resolve_project_exe_key(project_dir: &Path) -> Option<[u8; 16]> {
    ...
    // Fast path: a key that already started this exact resource set is trusted
    // outright. This runs before the resource files are read, so a trusted
    // install does not even pay for the Scene.pck read, the structural
    // validation (which false-negatives on repacked resources) or the full
    // recovery that follows it.
    if let Some(key) = configured_key {
        if key_trust_matches(project_dir, &key) {
            log::warn!(
                "[SG_KEYTRUST] trusting previously working key for {}; skipping key validation and recovery",
                project_dir.display()
            );
