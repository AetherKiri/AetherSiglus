...
/// Resolve the optional per-game Siglus EXE key. Games distributed with an
/// encrypted `Gameexe.dat`/`Scene.pck` pair may not ship a `key.toml`; in that
/// case recover the key from the resource headers and cache it for this run.
/// A recovered key is also persisted as a sidecar so subsequent launches do
/// not repeat the expensive resource crack.
#[cfg(not(all(target_arch = "wasm32", target_os = "unknown")))]
